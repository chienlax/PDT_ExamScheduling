...
void Process::Test(string inputFile)
{
	ifstream instream(inputFile);
	if (!instream.is_open()) {
		cout << "Unable to open file\n";
	...
	getline(instream, input); //Remove first line of the csv file
	int input_line = 0;
	cout << "Test preprocessing file start here" << endl;
	//cout << "Number of input line: " << input_line << endl;
	...
	while (getline(instream, input)) {
		...
		if (lines[4] == "TXGDTC0003" || lines[4] == "TXGDTC0001" || lines[4] == "TXGDTC0004" || lines[4] == "TXGDTC0002") {
			continue;
		}
